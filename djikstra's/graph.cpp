    ...
    for (int i = 0; i < this->n; i++) {
      ...
      if (hasBFSPath(curr, i) != 0) {
        ...
        if (!(visited[i])) {
          ...
          info.push_back(
              edge(this->v[curr], this->v[i], (this->adjMat[curr][i]).first));

          // add vertex and distance to order vector
          this->order.push_back(
              std::make_pair(this->v[i], (this->adjMat[curr][i]).first));
...
/*void graph::printDiscoveredEdges() {
    ...
    for(const auto& x : this->discoveredEdges) {
        ...
        std::cout << x.start.city << " and " << x.end.city << ": " << x.weight
<< " mi" << std::endl;
  ...
  for (int i = 0; i < this->n; i++) {
    for (int j = 0; j < this->n; j++) {
      if (((this->adjMat[i][j]).second == 0) &&
        ...
        std::cout << (this->v[i]).city << " and " << (this->v[j]).city << ": "
                  << (this->adjMat[i][j]).first << " mi" << std::endl;
      }
    }
  }
...
void graph::printLvlOrder() {
  ...
  for (int i = 0; i < this->order.size(); i++) {
    std::cout << "level " << lvl << ": " << this->order[i].first.city
              << std::endl;
...
void graph::printTotalDist() {
  ...
  std::cout << "total distance traveled: " << this->distanceInfo.second << " mi"
            << std::endl;
}
...
// printDjikstra()
void graph::printDjikstra(const int dist[], const std::string &name,
                          int path[]) {
  ...
  for (int i = 0; i < this->n; i++) {
    ...
    std::cout << name << " to " << this->v[i].city << ": " << dist[i] << " mi"
              << std::endl;
    ...
    // [start->next->next->...->end]
    std::cout << "path from " << name << " to " << this->v[i].city << ": "
              << pathStr.substr(0, pathStr.length() - 2) << std::endl
              << std::endl;
  }
}
...
// MST
void graph::MST(const std::string &name) {
  // total mileage
  int total = 0;
  ...
  std::cout << "Total mileage: " << total << " mi";
}

// print MST as while loop happens
void graph::printMST(int x, int y)
{  
          std::cout << this->v[x].city << "--->" << this->v[y].city << " : "
              << adjMat[x][y].first << " mi";
